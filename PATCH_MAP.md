...
*B12 and B13 (T6.10) are different in kind from the rest: they add NEW files*
*rather than changing upstream's, because the compile half of our release*
*pipeline now runs in this repository. They cannot conflict textually — but*
*they are the two rows most likely to be forgotten in a merge, because nothing*
*upstream ever touches the paths they occupy.*
...
| B12 | applied 2026-09-19 | **new** `.github/workflows/tracemote-build.yml` | the whole compile matrix — bridge, notices, macOS ×2, Linux, Windows — building **unbranded and unsigned**, publishing the result as a prerelease on this public fork | T6.10. The private repo's matrix bills ~1,230 min/run against a 3,000/month quota (macOS 10x, Windows 2x) and ran out. GitHub does not meter standard runners on public repos, and this fork is public because AGPL §6(d) requires it. Nothing secret is needed to compile: `custom.txt` is runtime data, not a compile input (BRANDING.md §3), so the split falls on a line that already existed | Isolated (new file) | low — upstream owns 11 workflows and none is named this. **But**: it duplicates upstream's `flutter-build.yml` pins (Flutter, Rust, vcpkg, NASM). When upstream bumps one, this file does not follow on its own |
| B13 | applied 2026-09-19 | **new** `scripts/release-verify.sh` (moved here from `tracemote/tracemote`, which keeps a one-line exec shim at the same path) | the artifact verifier, plus a new `TRACEMOTE_VERIFY_STAGE` env (`compile` \| `release`) — `compile` skips the `custom.txt` and signature checks, which cannot pass before injection — and **platform detection from the artifact rather than `uname`**, so a Linux or Windows bundle can be verified from a Mac | T6.10. B12 runs in a repo where the parent does not exist, so a script at `../../scripts/` reaches nothing; a submodule can read its superproject only by accident of checkout layout. The alternative was a second copy, and two copies of a release gate drift in the direction of passing | Isolated (new file) | low — no upstream `scripts/` directory exists at all |
