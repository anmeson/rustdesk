...
| B13 | applied 2026-09-19 | **new** `scripts/release-verify.sh` (moved here from `tracemote/tracemote`, which keeps a one-line exec shim at the same path) | the artifact verifier, plus a new `TRACEMOTE_VERIFY_STAGE` env (`compile` \| `release`) — `compile` skips the `custom.txt` and signature checks, which cannot pass before injection | T6.10. B12 runs in a repo where the parent does not exist, so a script at `../../scripts/` reaches nothing; a submodule can read its superproject only by accident of checkout layout. The alternative was a second copy, and two copies of a release gate drift in the direction of passing | Isolated (new file) | low — no upstream `scripts/` directory exists at all |
